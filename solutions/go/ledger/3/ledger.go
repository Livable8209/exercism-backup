...
func FormatLedger(currency string, locale string, entries []Entry) (string, error) {
	...
	for _, entry := range entriesCopy {
		...
		timeFormatted, err := time.Parse(time.DateOnly, entry.Date)
		if err != nil {
	...
	finalString := make([]string, len(entriesCopy))
	for i, entry := range entriesCopy {
		entryIDX, entryString, entryError := bigUglyFunc(i, currency, locale, entry)
		if entryError != nil {
			return "", entryError
		}
		finalString[entryIDX] = entryString
	}
	for i := range len(entriesCopy) {
		formattedLedger += finalString[i]
	}
