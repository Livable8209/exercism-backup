...
func FormatLedger(currency string, locale string, entries []Entry) (string, error) {
	...
	for range entriesCopy {
		v := <-formatRows(currency, locale, entriesCopy)
		if v.e != nil {
			return "", v.e
		}
		ss[v.i] = v.s
...
func formatRows(currency string, locale string, entriesCopy []Entry) chan struct {
	i int
	s string
	e error
} {
	ledgerEntry := make(chan struct {
		i int
		s string
		e error
	})
		...
		go func(i int, entry Entry) {
			...
			if len(entry.Date) != 10 {
				ledgerEntry <- struct {
					...
					s string
					e error
				}{e: errors.New("date is not exactly 10 characters long")}
			}
			...
			if firstDash != '-' {
				ledgerEntry <- struct {
					...
					s string
					e error
				}{e: errors.New("first dash is not present in date")}
			}
			if secondDash != '-' {
				ledgerEntry <- struct {
					i int
					s string
					e error
				}{e: errors.New("second dash is not present in date")}
			}
			...
			if locale == "nl-NL" {
				...
				if currency == "EUR" {
					...
					ledgerEntry <- struct {
						...
						s string
						e error
					}{e: errors.New("unknown curreny type in NL locale")}
				}
				ledgerTransaction += " "
				...
				if currency == "EUR" {
					...
					ledgerEntry <- struct {
						...
						s string
						e error
					}{e: errors.New("unknown currency type in US locale")}
				}
				centsStr := strconv.Itoa(cents)
				switch len(centsStr) {
				case 1:
				...
				ledgerEntry <- struct {
					...
					s string
					e error
				}{e: errors.New("unknown locale when attempting to format transactions rows")}
			}
			...
			ledgerEntry <- struct {
				...
				s string
				e error
			}{i: i, s: ledgerDate + strings.Repeat(" ", 10-len(ledgerDate)) + " | " + entryDescription + " | " + strings.Repeat(" ", 13-al) + ledgerTransaction + "\n"}
		}(i, entry)
	}
	return ledgerEntry
}
